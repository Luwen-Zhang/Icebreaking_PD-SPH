...
Real resolution_ref = JR / 10; /**< Initial reference particle spacing. */
Real BW = resolution_ref * 4.0;			/**< Extending width for BCs. */

/** Domain bounds of the system. */
BoundingBox system_domain_bounds(Vec2d(-BW, -BW), Vec2d(ID + BW, RL + IT + JH + BW));
/**
 ...
 */
Vec2d PlateP_lb(0.0, RL); 					/**< Left bottom. */
Vec2d PlateP_lt(0.0, RL + IT);				/**< Left top. */
Vec2d PlateP_rt(ID, RL + IT); 				/**< Right top. */
Vec2d PlateP_rb(ID, RL); 					/**< Right bottom. */
...
public:
	explicit WaterBlock(const std::string &shape_name) : MultiPolygonShape(shape_name)
	{
		...
		water_block_shape.push_back(JetP_lb);
		water_block_shape.push_back(JetP_lt);
		water_block_shape.push_back(JetP_rt);
		water_block_shape.push_back(JetP_rb);
		water_block_shape.push_back(JetP_lb);
...
public:
	explicit WallBoundary(const std::string &shape_name) : MultiPolygonShape(shape_name)
	{
		...
		left_wall_shape.push_back(Vecd(0.0, 0.0));
		left_wall_shape.push_back(Vecd(0.0, RL));
		left_wall_shape.push_back(Vecd(RL, RL));
		left_wall_shape.push_back(Vecd(RL, 0.0));
		left_wall_shape.push_back(Vecd(0.0, 0.0));

		std::vector<Vecd> right_wall_shape;
		right_wall_shape.push_back(Vecd(ID - RL, 0.0));
		right_wall_shape.push_back(Vecd(ID - RL, RL));
		right_wall_shape.push_back(Vecd(ID, RL));
		right_wall_shape.push_back(Vecd(ID, 0.0));
		right_wall_shape.push_back(Vecd(ID - RL, 0.0));

		multi_polygon_.addAPolygon(left_wall_shape, ShapeBooleanOps::add);
		multi_polygon_.addAPolygon(right_wall_shape, ShapeBooleanOps::add);
	}
};
//----------------------------------------------------------------------
...
int main()
{
	...
	SurfaceContactRelation plate_restrictor_contact(plate, { &wall_boundary });
	//----------------------------------------------------------------------
	...
	SimpleDynamics<solid_dynamics::PDTimeStepInitialization> initialize_a_solid_step(plate, makeShared<Gravity>(Vecd(0.0, -gravity_g)));
	...
	InteractionDynamics<solid_dynamics::ContactDensitySummation> plate_update_contact_density(plate_restrictor_contact);
	InteractionDynamics<solid_dynamics::ContactForce> plate_compute_solid_contact_forces(plate_restrictor_contact);
	//----------------------------------------------------------------------
