...
Real PH = 0.06; // for thick plate; =0.01 for thin plate
Real BH = 0.1;  // Ball Height
Real BBD = 0.002;//Beam-Ball distance
// reference particle spacing
Real resolution_ref = PH / 30.0;
Vec2d ball_center(PL / 2.0, BH);
Real ball_radius = PL / 5.0;
/** Domain bounds of the system. */
BoundingBox system_domain_bounds(Vec2d(-3.0 * resolution_ref, -3.0 * resolution_ref),
	Vec2d(PL + 3.0 * resolution_ref, BH + ball_radius + BBD + PH + 3.0 * resolution_ref));
//----------------------------------------------------------------------
...
std::vector<Vecd> beam_shape{
	Vecd(0.0, BH + ball_radius + BBD), Vecd(0.0, BH + ball_radius + BBD + PH), 
	Vecd(PL, BH + ball_radius + BBD + PH), Vecd(PL, BH + ball_radius + BBD), 
	Vecd(0.0, BH + ball_radius + BBD)};
...
int main(int ac, char *av[])
{
	...
	SolidBody ball(system, makeShared<GeometricShapeBall>(ball_center, ball_radius, "SolidBall"));
	ball.defineParticlesAndMaterial<SolidParticles, NeoHookeanSolid>(rho0_s, Youngs_modulus, poisson);
	...
	if (system.RunParticleRelaxation())
	{
		...
		ReloadParticleIO write_particle_reload(io_environment, { &ball });
		//----------------------------------------------------------------------
	...
	SurfaceContactRelation beam_ball_contact(beam_body, { &ball });	
	//-----------------------------------------------------------------------------
	...
	beam_shapeMatrix.parallel_exec();
	...
	while (GlobalStaticVariables::physical_time_ < end_time)
	{
		...
		while (integration_time < output_interval)
		{
			...
			while (relaxation_time < Dt)
			{
				...
				beam_update_contact_density.parallel_exec(dt);
				...
				beam_ball_contact.updateConfiguration();
